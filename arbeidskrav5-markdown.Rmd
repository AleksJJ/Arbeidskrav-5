...
str.m2 <- lmerTest::lmer(post ~ pre + sex + sets +(1|participant), data = dat)
str.m3 <- lm(post ~ pre + sex + sets, data = dat)

summary(str.m1)


tabellstyrke <- str.dat %>%
mutate(sets = factor(sets, levels = c("multiple", "single"), 
                        labels = c("3 sett", "1 sett"))) %>%
  select(sets, pre, post) %>%
  arrange(sets) %>%
   flextable()  %>%
  set_header_labels(sets = "pre",
                    post = "post") %>%
